...
class GraphSatTw(object):
    def __init__(self, G, timeout=0, stream=sys.stdout):
        self.G = G
    ...
    def improved(self, n):
        ...
        # if u and v have a common predecessor then the hypergraph contains the edge {u,v} (5)
        for i in xrange(1, n + 1):
            for j in xrange(1, n + 1):
                if i == j:
                    continue
    ...
    # hypergraph=contract(hypergraph,5)
    # print hypergraph.number_of_nodes(), hypergraph.number_of_edges()
    ...
    def minor_lbound(self, c, k):
        # Assume we believe that tw(hypergraph)> k since we hit a timeout for k.
        ...
        # then we have confirmed tw(hypergraph)> k as by contraction of edges we obtained a minor of hypergraph.
